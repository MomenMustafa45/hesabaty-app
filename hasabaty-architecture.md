...
**`RecentTransactionsList`** — last 5 transactions, sorted by date descending. Each row: a colored circle showing the category's **first letter** (matching `.cat-dot` exactly) — not a real icon. `AppIcon`'s 21 icons are UI/navigation glyphs (home, settings, chevrons, etc.), none are category-specific (no food/transport/electricity icons exist) — don't invent new icon art to fill this gap, the letter-avatar approach is the actual prototype behavior, not a placeholder waiting to be upgraded.
